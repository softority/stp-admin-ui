...
@Component({
  selector: 'stp-tests-tree',
  templateUrl: './tests-tree.component.html',
  styleUrls: ['./tests-tree.component.scss']
})
export class TestsTreeComponent {
    ...
    if (node.type === 'folder') {
      return {
        items: [
          ...
          { name: 'Add test', action: this.addTest.bind(this) },
      ...
      return {
        items: [
          //{ name: 'Add test', action: this.addTest.bind(this) },
          { name: 'Rename', action: this.renameTest.bind(this, node) },
          { name: 'Edit', action: this.editTest.bind(this) },
          { name: 'Delete', action: this.deleteTest.bind(this, node) },
        ]
      }
    }
  }
  ...
  private renameTest(node: FlatTreeNode) {
    this.openPromptDialog(
      {
        title: 'Rename test',
        placeholder: 'Name',
  ...
  private deleteTest(node: FlatTreeNode) {
    this.openConfirmationDialog(
      {
        title: 'Delete test ' + node.name + '. Are you shure?'
      })
      .subscribe(res => {
        if (res !== undefined) {
          // TODO
